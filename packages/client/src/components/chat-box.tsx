...
const ChatBox: React.FC<Props> = React.memo(
  ({ roomID }) => {
    const [message, setMessage] = React.useState("");
    const [messages, setMessages] = React.useState<Pick<Message, "content" | "user" | "messageID">[]>([]);
    ...
    return (
      ...
      >
        {messages.length <= 0 && (
          <div className="p-4 text-center"> There are no messages yet. Send one?</div>
        )}
        ...
        <div className="bg-accent/20 rounded-lg flex items-center border focus-within:ring-1 focus-within:ring-foreground/50 focus-within:ring-offset-2 focus-within:ring-offset-background">
          ...
          <div className="h-full">
            <Button
              ...
              type="submit"
            >
              <svg width={16} height={16} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
