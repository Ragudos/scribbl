			...
			{sizes.map((s) => (
				<Toolbar.Button key={s} asChild>
					<Button
						...
						variant="ghost"
					>
						<MiniCanvas color={color} size={s} lineCap={lineCap} />
					</Button>
				</Toolbar.Button>
			))}
		</Toolbar.ToggleGroup>
		...
		<Toolbar.ToggleGroup className="flex gap-2 items-center" type="multiple">
			<Toolbar.Button asChild>
				<Button
					variant="ghost"
					size="icon"
					className={cn("p-1 w-8 h-8 border", {
		...
		<Toolbar.ToggleGroup className="flex items-center gap-2" type="multiple">
			{lineCaps.map((l) => (
				<Toolbar.Button key={l} asChild>
					<Button
						size="icon"
						className={cn("p-1 w-8 h-8 border hover:bg-accent/20", {
					...
					>
						<MiniCanvas color={color} lineCap={l} size={10} />
					</Button>
				</Toolbar.Button>
			))}
		</Toolbar.ToggleGroup>
...
const MiniCanvas: React.FC<{
	...
	return <canvas ref={ref} width={size} height={size}></canvas>;
});
