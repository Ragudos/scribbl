...
const ReadOnlyRoomSettings = React.lazy(
	() => import("./read-only-room-settings"),
);
...
const WaitingRoom: React.FC<Props> = React.memo(
	({ roomID, roomOwnerID, maxPlayerAmount, maxRounds, playersLength }) => {
		const [maxPlayerAmountState, setMaxPlayerAmount] =
			React.useState<MAX_PLAYERS_PER_ROOM>(maxPlayerAmount);
		const [maxRoundsState, setMaxRounds] =
			React.useState<MAX_ROUNDS>(maxRounds);
				...
				<div className="flex flex-col gap-8 shadow-md shadow-black/20 dark:border rounded-lg p-4">
					...
					{roomOwnerID === socket.id && (
						<div>
							...
							>
								{playersLength <= 1
									? "There must be more than one player to start"
									: "Start game"}
							</Button>
						</div>
					)}
					...
					{roomOwnerID !== socket.id && (
						<div>Waiting for the room owner to start the game...</div>
					)}
				</div>
			</React.Fragment>
		);
