...
type Props = {
	messages: Pick<
		Message,
		"content" | "user" | "messageID" | "isSystemMessage"
	>[];
};

const Chat: React.FC<Props> = React.memo(({ messages }) => {
				...
				<React.Fragment>
						...
						>
							...
							{m.user.userID !== socket.id && (
								<div className="text-sm max-w-[70%] md:max-w-[50%] relative">
									{m.user.userID !== socket.id && (
										<span className="whitespace-nowrap text-sm bottom-[110%] left-0 absolute opacity-60 font-bold">
											{m.user.displayName}
										</span>
									)}
					...
					{m.user === null && m.isSystemMessage && (
						<li
							ref={lastMsgRef}
							key={m.messageID}
							className="flex justify-center opacity-60 text-sm text-center w-full"
						>
							<div className="max-w-sm overflow-auto break-words whitespace-normal">
								{m.content}
							</div>
						</li>
					)}
